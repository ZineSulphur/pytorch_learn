...
公式计算可以得到 $pos_x$ 和 $pos_y$ 对应得到两个128维向量，拼接起来就是256x25x34(d,h,w)的张量。

相加之后将向量的h和w拉直成850x256(hw,d)的向量，将其输入到transformer-encoder中，输出850x256的结果。

![pe](./img/5.gif)

那为什么要位置编码呢？其实之后我们输入的张量需要拉平才能输入到Transformer中，这个动作会使其失去位置的信息，故需要加上位置编码。
...
即对于某一个真值 $y_i$ ，假设我们已经找到这个真值对应的预测值 $\hat{y}_ {\sigma(i)}$ ，这里的 $\mathfrak{G}_ N$ 是所有可能的排列，代表从真值索引到预测值索引的所有的映射，然后用 $\mathcal{L}_ {macth}$ 最小化 $y_i$ 和 $\hat{y}_{\sigma(i)}$ 的距离。
...
[详解DETR （End-to-End Object Detection with Transformers）](https://blog.csdn.net/weixin_43662553/article/details/126473883)

[DETR的一些说明](https://zhuanlan.zhihu.com/p/376311764)
